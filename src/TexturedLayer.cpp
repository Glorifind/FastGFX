...
namespace fgfx {
  ...
  TexturedLayer::TexturedLayer(Engine* enginep) : Layer(enginep) {
    composition=1;
  };
  ...
  void TexturedLayer::beginDraw(glm::mat4 cameraMatrix) {
    ...
    glUniformMatrix4fv(TexturedLayer::texturedProgramUniformCameraMatrix, 1, GL_FALSE, (GLfloat*)glm::value_ptr(cameraMatrix));
  }
  ...
  void TexturedLayer::render(glm::mat4 cameraMatrix) {
    if(!visible) return;
